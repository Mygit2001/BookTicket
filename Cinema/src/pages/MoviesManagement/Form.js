...
export default function FormInput({ selectedPhim, onUpdate, onAddMovie }) {
  ...
  const movieSchema = yup.object().shape({
    ...
    thoiLuong: yup.string().required("*Không được bỏ trống!"),
    ...
    >{(formikProp) => (
      <Form >
        ...
        <div className="form-group">
          ...
          <ErrorMessage name="theLoai" render={msg => <span className="text-danger">{msg}</span>} />
          <Field name="theLoai" className="form-control" />
        </div>
        <div className="form-group">
          <label>Quốc gia&nbsp;</label>
          <ErrorMessage name="quocGia" render={msg => <span className="text-danger">{msg}</span>} />
          <Field name="quocGia" className="form-control" />
        </div>
        <div className="form-group">
          <label>Thời lượng&nbsp;</label>
          <ErrorMessage name="thoiLuong" render={msg => <span className="text-danger">{msg}</span>} />
          <Field name="thoiLuong" className="form-control" />
        </div>
        <div className="form-group">
        ...
        <button type="submit" className="btn-submit form-control">Xác nhận</button>
      </Form>
    )}</Formik>
  )
}
